// Adapted copy: AllIn1 Sprite. Original plugin remains unchanged.
Shader "MonsterSupergroup/PlanarSprite"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			#include "Assets/Plugins/AllIn1SpriteShader/Shaders/AllIn1OneShaderFunctions.cginc"
            #include "Assets/_Project/Content/Rendering/Planar/GameplayPlanarProjection.hlsl"
            ...
            v2f vert (appdata v)
            {
				...
				o.vertex = mul(UNITY_MATRIX_VP, float4(GameplayPlanarWorld(mul(unity_ObjectToWorld, float4(localPos, 1)).xyz), 1));
				#else
				o.vertex = mul(UNITY_MATRIX_VP, float4(GameplayPlanarWorld(mul(unity_ObjectToWorld, v.vertex).xyz), 1));
				#endif
