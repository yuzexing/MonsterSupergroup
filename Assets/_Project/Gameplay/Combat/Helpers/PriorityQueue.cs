...
namespace AstralShift.Helpers
{
	    ...
	    ICollection
	{
	        ...
	        IDisposable
	    {
	        ...
	        public bool MoveNext()
	        {
	            ...
	            if (_version == queue._version &&
	                (uint)_index < (uint)queue._size)
	            {
	                Current = queue._nodes[_index];
	                _index++;
	                return true;
	            }
	    ...
	    void ICollection.CopyTo(Array array, int index)
	    {
	        ...
	        if (array.Rank != 1)
	            throw new ArgumentException(
	                "Multi-dimensional arrays not supported.", "array");

	        if (array.GetLowerBound(0) != 0)
	            throw new ArgumentException(
	                "Non-zero lower bound required.", "array");

	        if (index < 0 || index > array.Length)
	            throw new ArgumentOutOfRangeException(
	                "index", index, "Index must be less or equal");
		...
		public PriorityQueue(int initialCapacity, IComparer<TPriority>? comparer)
		{
			...
			{
				throw new ArgumentOutOfRangeException("initialCapacity", initialCapacity, "Non-negative number required.");
			}
