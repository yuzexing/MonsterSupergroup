...
namespace MonsterSupergroup.NetworkCombat
{
    ...
    public sealed partial class LimboReferenceLaunch : MonoBehaviour
    {
        ...
        public static bool SuppressDebugPanels => Profile == "effects-observe" || Profile == "movement-observe" || Profile.StartsWith("pickup-", StringComparison.Ordinal) || Profile.StartsWith("audio-", StringComparison.Ordinal) || Manual || Enabled && Argument("--limbo-performance-preset=") != null;
        public static bool Light { get; } = Argument("--limbo-log-detail=") == "light";
        public static bool Enabled => MonsterSupergroup.Builds.BuildFeatures.DevelopmentToolsAllowed && Argument("--limbo-role=") != null;
        ...
        private static void Install()
        {
            ...
            if (Profile == "movement-observe") runner.gameObject.AddComponent<EnemyMotionObservation>();
            if (Profile == "effects-observe") runner.gameObject.AddComponent<WeaponEffectsObservation>();
        ...
        private void OnGUI()
        {
            if (Manual && failure == null)
            {
                GUI.Label(new Rect(12, Screen.height - 24, Screen.width - 24, 24),
                    $"Limbo {MonsterSupergroup.Builds.RuntimeBuildInfo.Display} | {Argument("--limbo-session=")} | {role} | round {auditRound}");
                return;
            }
