    ...
    public sealed class PlanarPresentationTests
    {
        ...
        public IEnumerator PerspectiveCameraRendersOppositeDepthsAtTheSamePlanarSize()
        {
            ...
            try
            {
                ...
                foreach (string shader in new[] { "MonsterSupergroup/PlanarSprite", "MonsterSupergroup/PlanarVfx", "MonsterSupergroup/PlanarSummonShadow" })
                {
                material = new Material(Shader.Find(shader));
                bool shadow = shader.EndsWith("Shadow");
                camera.backgroundColor = shadow ? Color.white : Color.black;
                if (material.HasProperty("_Color")) material.SetColor("_Color", shadow ? Color.black : Color.white);
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    int min = 512, max = -1;
                    for (int x = 0; x < 512; x++) if (shadow ? image.GetPixel(x, 256).maxColorComponent < .9f : image.GetPixel(x, 256).maxColorComponent > .1f) { min = Mathf.Min(min, x); max = x; }
                    widths[i] = max - min + 1;
                }
                ...
                TestContext.WriteLine($"{shader}: near/far pixel widths={widths[0]}/{widths[1]}");
