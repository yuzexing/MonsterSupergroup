...
namespace MonsterSupergroup.Gameplay.Tests
{
    ...
    public sealed class WispNetworkProcessProbe : MonoBehaviour
    {
        ...
        private WispProcessTarget target;
        ...
        private static void Install()
        {
            ...
            string value = args.FirstOrDefault(a => a.StartsWith("--wisp-network-role="));
            if (value == null || FindFirstObjectByType<WispNetworkProcessProbe>() != null) return;
            var probe = new GameObject("Wisp Process Validation").AddComponent<WispNetworkProcessProbe>();
        ...
        private void Update()
        {
            ...
            {
                Debug.LogError("[WispNetwork] Timed out in role " + role);
                Finish(false);
            }
        }
        ...
        private IEnumerator Run()
        {
            ...
            Require(database != null && database.TryGetWeaponData(2, out var wisp), "Migrated wisp is absent from the weapon database.");
            // Only the test's in-memory definition changes: deterministic three-projectile damage and a long cooldown.
            var weapons = Instantiate(database.WeaponDB);
            weapons.Configure(weapons.Weapons.Select(definition =>
            {
                if (definition.ID != 2) return definition;
                var copy = Instantiate(definition);
                var behaviour = Instantiate(definition.WeaponPrefab.gameObject);
                behaviour.SetActive(false);
                copy.WeaponPrefab = behaviour.GetComponent<ProjectileAttackBehaviour>();
                typeof(ProjectileAttackBehaviour).GetField("hitCount", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(copy.WeaponPrefab, 1);
            ...
            manager.playerPrefab.GetComponent<PlayerBuildRuntime>().ConfigureInitialWeapon(2);
            ...
            Require(!FindObjectsByType<AnimatedAttack>(FindObjectsSortMode.None).Any(a => a.gameObject.activeInHierarchy), "Active projectile survived scene shutdown.");
            Mark("stopped-" + role);
        }
        ...
        private IEnumerator VerifyServer()
        {
            ...
            {
                Require(player.GetComponent<PlayerBuildRuntime>().InitialWeapon is ProjectileAttackBehaviour, "Server Build has the wrong weapon family.");
            ...
            foreach (var player in players)
            {
                ...
                while (!gateway.Ledger.TryGetState(TargetId(player.netId), out var state) || state.Health != 955 || gateway.Attacks.ActiveCount(player.netId) != 0)
                    yield return null;
                var adapter = player.GetComponent<NetworkWeaponCombatAdapter>();
                Require(adapter.AcceptedCooldownReportCount == 1 && adapter.RejectedAttackCount == 0, "Three projectiles must use one admitted root.");
                Require(adapter.RejectedPresentationCount == 0, "Server rejected a legal projectile edge.");
                Debug.Log($"[WispNetwork] canonical player={player.netId} hp=955 roots=0 admitted=1");
            }
            ...
            // Use a second genuinely legal root, then disconnect while its first projectile is alive.
            ...
            Require(retained.Checkpoint != null && retained.Checkpoint.WeaponCooldowns.Length == 1,
                "Disconnect lost its projectile cooldown checkpoint.");
            while (!Has("late-flight-" + (role == "host" ? "host" : "client2"))) yield return null;
            Mark("reconnect");
            ...
            Require(restored.InitialWeapon is ProjectileAttackBehaviour && restored.InitialWeapon.ProjectileCountValue == 3,
                "Reconnect did not restore the original wisp Build.");
            Debug.Log($"[WispNetwork] reconnect participant={participantId} oldAvatar={oldAvatar} newAvatar={retained.AvatarId} sequenceRetained=true");
            Mark("stop");
        ...
        private IEnumerator VerifyOwner()
        {
            ...
            var weapon = build.InitialWeapon as ProjectileAttackBehaviour;
            Require(weapon != null, "Owner did not receive the wisp Build.");
            var targetObject = new GameObject("Wisp physics target");
            ...
            target = targetObject.AddComponent<WispProcessTarget>();
            target.Initialize(TargetId(owner.netId));
            weapon.Attack();
            float end = Time.realtimeSinceStartup + 3;
            while (Time.realtimeSinceStartup < end) yield return null;
            Require(target.HitCount == 3 && target.TotalDamage == 45, $"Physics hits must be 3 x 15; got {target.HitCount} hits, {target.TotalDamage} damage.");
            ...
            Require(replica.ReceivedPresentationCount == 6 && replica.ReplicaActiveProjectileCount == 0,
                $"Remote must replay three spawns/ends and release every projectile; received={replica.ReceivedPresentationCount}, active={replica.ReplicaActiveProjectileCount}.");
            Require(replica.ReplicaSpawnCount == 3 && replica.RejectedPresentationCount == 0,
                $"Every projectile must actually be replayed; spawned={replica.ReplicaSpawnCount}, rejected={replica.RejectedPresentationCount}.");
            ...
            Debug.Log($"[WispNetwork] owner={owner.netId} hits=3 damage=45 remoteEdges=6 remoteActive=0");
            Destroy(targetObject);
            Mark("verified-" + role);
            if (role == "host" || role == "client2")
            {
                while (!Has("disconnected-client")) yield return null;
                weapon.Attack();
                while (owner.GetComponent<NetworkWeaponCombatAdapter>().AcceptedCooldownReportCount < 2 && role == "host") yield return null;
                yield return new WaitForSeconds(.2f);
                Mark("late-flight-" + role);
            }
        }
        ...
        private IEnumerator DisconnectDuringBurstAndReconnect()
        {
            ...
            var weapon = (ProjectileAttackBehaviour)oldPlayer.GetComponent<PlayerBuildRuntime>().InitialWeapon;
            weapon.Attack();
            while (!Has("burst-admitted")) yield return null;
            yield return new WaitForSeconds(0.04f);
            Require(FindObjectsByType<ProjectileAttack>(FindObjectsSortMode.None).Any(p => !p.PresentationOnly), "Fixture must disconnect with an active projectile.");
            manager.StopClient();
            ...
            Require(!FindObjectsByType<AnimatedAttack>(FindObjectsSortMode.None).Any(a => a.gameObject.activeInHierarchy),
                "Disconnect left an active owned or remote projectile.");
            ...
            var wisp = restored.GetComponent<PlayerBuildRuntime>().InitialWeapon;
            Require(wisp is ProjectileAttackBehaviour && wisp.ProjectileCountValue == 3 &&
                wisp.GetCooldown() - wisp.LastAttackElapsedTime > 10f, "Reconnect reset the wisp Build or remaining cooldown.");
            Debug.Log($"[WispNetwork] resumed oldAvatar={oldAvatar} newAvatar={restored.netId} remaining={wisp.GetCooldown() - wisp.LastAttackElapsedTime:F3}");
            var remote = NetworkClient.spawned.Values.First(p => p != null && !p.isOwned && p.GetComponent<NetworkRunParticipant>() != null);
            float until = Time.realtimeSinceStartup + 5f;
            while (remote.GetComponent<NetworkWeaponCombatAdapter>().ReplicaActiveProjectileCount < 3 && Time.realtimeSinceStartup < until) yield return null;
            Require(remote.GetComponent<NetworkWeaponCombatAdapter>().ReplicaActiveProjectileCount == 3, "Late observer did not restore live projectiles");
            var visual = FindObjectsByType<ProjectileAttack>(FindObjectsSortMode.None).Where(p => p.PresentationOnly).ToArray();
            Require(visual.Length == 3 && visual.All(p => (bool)typeof(ProjectileAttack).GetField("_suppressLaunchSound", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).GetValue(p)), "Historical projectile replayed its shot");
            Debug.Log("[WispNetwork] restored=3 historicalShotsSuppressed=true");
            Mark("resumed-client");
        }
        ...
        private void VerifyRemoteDisconnectCleanup()
        {
            ...
            Require(!ReferenceEquals(firstRemoteAdapter, null) && firstRemoteAdapter.ReplicaSpawnCount > 3,
                "Spectator must have replayed the disconnecting player's second attack.");
            Require(firstRemoteAdapter.ReplicaActiveProjectileCount == 0, "Disconnect left the old Avatar's projectile replica active.");
        }
        ...
        private void Finish(bool passed)
        {
            ...
            finished = true;
            SceneManager.sceneLoaded -= PrepareGameplay;
            Debug.Log("[WispNetwork] result=" + (passed ? "PASS" : "FAIL") + " role=" + role);
            Application.Quit(passed ? 0 : 1);
        }
    }

    public sealed class WispProcessTarget : MonoBehaviour, IDamageable, INativeGasDamageable
    {
