...
async function query(){const serial=++querySerial;selection=readSelection();cursor=null;selected=null;$("detail").replaceChildren();$("detail-panel").querySelector(".empty").hidden=false;$("events").replaceChildren();message("正在查询完整归档…");const result=await api("timeline",{selection});if(serial!==querySerial)return;addEvents(result.items||[]);cursor=result.nextCursor;$("more").hidden=!cursor;$("count").textContent="符合条件共 "+(result.total??"未知")+" 条；当前展示 "+(result.items||[]).length+" 条";$("page-note").textContent=cursor?"继续加载可查看全部结果，筛选作用于整个归档。":"已展示此筛选的全部结果。";if(!(result.items||[]).length)$("events").append(node("p","没有找到符合条件的记录；未记录不表示未发生。","empty"));const [coverage,tracks]=await Promise.all([api("coverage",{selection}),api("tracks",{selection})]);if(serial!==querySerial)return;renderCoverage(coverage);renderTracks(tracks);message("查询完成。点击事件展开调查过程。");}
