param(
    ...
    [ValidateSet('opening', 'full', 'imp', 'imp-validation', 'imp-v0', 'imp-v1','stage2','stage2-validation','stage2-fixture','spatial-b','spatial-barrier','spatial-overlap','spatial-reposition','dash','dash-fixture','dash-validation')][string]$Profile = 'opening',
    ...
    [ValidateSet('mechanism','melee-cleanup','l-hold','l-kill','l-boundary')][string]$FixtureMode = 'mechanism',
...
$launchArgs = @("-force-$GraphicsApi", '-screen-fullscreen', '0', '-screen-width', '1100', '-screen-height', '700',
    ...
    ('"--limbo-output=' + $outputDirectory + '"'), "--limbo-autowalk=$($AutoWalk.IsPresent.ToString().ToLowerInvariant())", "--limbo-windowed=$($Windowed.IsPresent.ToString().ToLowerInvariant())")
