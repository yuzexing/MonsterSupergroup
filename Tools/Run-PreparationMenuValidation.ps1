...
function Launch([string]$role) {
    ...
    $processes[$role] = Start-Process -FilePath $Executable -WindowStyle $windowStyle -PassThru -ArgumentList $arguments
...
try {
    ...
    foreach ($process in $processes.Values) { if (-not $process.HasExited) { Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue } }
}
