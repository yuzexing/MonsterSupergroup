...
$logRoot = Join-Path (Join-Path $projectRoot $LogDirectory) "Circling-$modeName-$Port-$stamp"
New-Item -ItemType Directory -Path $logRoot -Force | Out-Null
$processes = @{}
function Launch([string]$role) {
    $arguments = @('-batchmode','-nographics','-logFile',"`"$logRoot/$role.log`"",
        "--circling-role=$role","`"--circling-sync=$logRoot`"","--circling-port=$Port")
...
try {
    ...
    Write-Output "Circling validation passed: $logRoot"
} finally {
