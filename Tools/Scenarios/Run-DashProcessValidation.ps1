param(
    [string]$Executable = 'Builds/Phase02/DashValidation.exe',
    [switch]$Dedicated,
    [int]$Port = 7954,
...
if ($Dedicated -and -not $PSBoundParameters.ContainsKey('Port')) { $Port = 7955 }
...
$logRoot = Join-Path (Join-Path $projectRoot $LogDirectory) "Dash-$modeName-$Port-$stamp"
New-Item -ItemType Directory -Path $logRoot -Force | Out-Null
$processes = @{}
function Launch([string]$role) {
    $arguments = @('-batchmode','-nographics','-logFile',"`"$logRoot/$role.log`"",
        "--dash-role=$role","`"--dash-sync=$logRoot`"","--dash-port=$Port")
...
try {
    ...
    Write-Output "Dash validation passed: $logRoot"
} finally {
    foreach ($process in $processes.Values) {
        if ($process -and -not $process.HasExited) { Stop-ProjectProcess -Id $process.Id -Force }
    }
}
