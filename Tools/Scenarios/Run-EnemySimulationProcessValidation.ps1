...
$Executable = Resolve-ProjectBuildExecutable -ProjectRoot $projectRoot -Recipe 'sandbox' -Executable $Executable -RequireDevelopmentTools -Network Kcp
...
try {
    ...
    if ($clientProcess -ne $null -and -not $clientProcess.HasExited) {
        Stop-ProjectProcess -Id $clientProcess.Id -Force
    }
    if ($hostProcess -ne $null -and -not $hostProcess.HasExited) {
        Stop-ProjectProcess -Id $hostProcess.Id -Force
    }
}
