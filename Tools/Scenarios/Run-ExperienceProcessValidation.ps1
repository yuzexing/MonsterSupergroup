...
if (-not (Test-Path -LiteralPath $Executable)) { throw "Missing M6 build: $Executable" }
...
function Launch([string]$role) {
    $arguments = @('-logFile', ('"' + $logRoot + '/' + $role + '.log"'), "--m6-role=$role",
    ...
    $processes[$role] = Start-ProjectProcess -FilePath $Executable -ArgumentList $arguments -WindowStyle $style -PassThru
...
function AwaitMarker([string]$marker) {
    ...
    while (-not (Test-Path -LiteralPath (Join-Path $logRoot $marker))) {
        if ((Get-Date) -gt $until -or @($processes.Values | Where-Object HasExited).Count -gt 0) { throw "M6 failed waiting for $marker : $logRoot" }
...
try {
    ...
    while (@($processes.Values | Where-Object { -not $_.HasExited }).Count -gt 0) {
        if ((Get-Date) -gt $until) { throw "M6 timed out: $logRoot" }
        if (@($processes.Values | Where-Object { $_.HasExited -and $_.ExitCode -ne 0 }).Count -gt 0) { throw "M6 process failed: $logRoot" }
        Start-Sleep -Milliseconds 250
    }
    foreach ($role in $processes.Keys) {
        Wait-ProjectProcess -Process $processes[$role]
        $code = $processes[$role].ExitCode
        Write-Output "M6 role=$role exit=$code"
        if ($code -ne 0 -or -not (Select-String -LiteralPath (Join-Path $logRoot ($role + '.log')) -SimpleMatch "[M6Process] result=PASS role=$role" -Quiet)) {
            throw "M6 gameplay validation failed for $role : $logRoot"
        }
    }
    Write-Output "M6 validation passed: $logRoot"
}
finally {
    foreach ($role in $processes.Keys) {
