...
try {
    foreach ($role in @('host', 'client')) {
        ...
        $process = Start-ProjectProcess -FilePath $Executable -WindowStyle $windowStyle -PassThru -ArgumentList $playerArguments
    ...
    foreach ($process in $processes) { Wait-ProjectProcess -Process $process }
    ...
    foreach ($process in $processes) { if (-not $process.HasExited) { Stop-ProjectProcess -Id $process.Id -Force } }
}
