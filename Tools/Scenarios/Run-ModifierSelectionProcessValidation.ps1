...
function Start-SelectionProcess([string]$Role) {
    ...
    Start-ProjectProcess -FilePath $Executable -WindowStyle $windowStyle -PassThru -ArgumentList $processArgs
}
...
try {
    ...
    foreach ($process in @($clientProcess, $hostProcess)) {
        if ($null -ne $process -and -not $process.HasExited) { Stop-ProjectProcess -Id $process.Id -Force }
    }
}
