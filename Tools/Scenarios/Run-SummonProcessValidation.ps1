param(
    [string]$Executable = 'Builds/Phase02/SummonValidation.exe',
    [switch]$Dedicated,
    [int]$Port = 7956,
...
$logRoot = Join-Path (Join-Path $projectRoot $LogDirectory) "Summon-$modeName-$Port-$stamp"
New-Item -ItemType Directory -Path $logRoot -Force | Out-Null
$processes = @{}
function Launch([string]$role) {
    $arguments = @('-batchmode','-nographics','-logFile',"`"$logRoot/$role.log`"",
        "--summon-role=$role","`"--summon-sync=$logRoot`"","--summon-port=$Port")
...
try {
    ...
    Write-Output "Summon validation passed: $logRoot"
} finally {
