...
if (-not (Test-Path -LiteralPath $Executable)) { throw "Missing M4 validation build: $Executable" }
...
$logRoot = Join-Path $projectRoot ('Logs/M4/' + $modeName + '-' + (Get-Date -Format 'yyyyMMdd-HHmmss-fff') + '-p' + $Port)
...
Write-Output "M4 process artifacts: $logRoot"
$processes = @{}
function Launch([string]$role) {
    $arguments = @('-logFile', ('"' + $logRoot + '/' + $role + '.log"'), "--m4-role=$role",
        ('"--m4-artifacts=' + $logRoot + '"'), "--m4-port=$Port", '-screen-fullscreen','0','-screen-width','1920','-screen-height','1080')
    if ($Dedicated) { $arguments += '--m4-dedicated' }
    if ($ForceD3D11) { $arguments += '-force-d3d11' }
    if ($CaptureFrames -and $role -ne 'server') { $arguments += '--m4-capture' }
    ...
    $processes[$role] = Start-ProjectProcess -FilePath $Executable -ArgumentList $arguments -WindowStyle $style -PassThru @launchOptions
...
function AwaitMarker([string]$marker) {
    ...
    while (-not (Test-Path -LiteralPath (Join-Path $logRoot $marker))) {
        if ((Get-Date) -gt $deadline -or @($processes.Values | Where-Object HasExited).Count -gt 0) { throw "M4 failed waiting for $marker : $logRoot" }
...
try {
    ...
    while (@($processes.Values | Where-Object { -not $_.HasExited }).Count -gt 0) {
        if ((Get-Date) -gt $deadline) { throw "M4 timed out: $logRoot" }
        if (@($processes.Values | Where-Object { $_.HasExited -and $_.ExitCode -ne 0 }).Count -gt 0) { throw "M4 player exited with an error: $logRoot" }
        Start-Sleep -Milliseconds 250
    }
    foreach ($role in $processes.Keys) {
        Wait-ProjectProcess -Process $processes[$role]
        $code = $processes[$role].ExitCode
        Write-Output "M4 role=$role exit=$code"
        Set-Content -LiteralPath (Join-Path $logRoot ($role + '.exit.txt')) -Value $code
        if ($code -ne 0 -or -not (Select-String -LiteralPath (Join-Path $logRoot ($role + '.log')) -SimpleMatch "[M4Process] result=PASS role=$role" -Quiet)) {
            throw "M4 validation failed for $role : $logRoot"
        }
    }
    Write-Output "M4 validation passed: $logRoot"
}
finally {
    foreach ($role in $processes.Keys) {
        ...
        } else { Stop-ProjectProcess -Id $process.Id -Force }
    }
}
