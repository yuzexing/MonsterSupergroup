...
if ($context.Metadata.stage -eq 'network') {
    # The copied case already keeps its original name; do not repeat it in the export parent.
    $output = Join-Path (Split-Path -Parent $context.Case) ('network-export-' + (New-CaseSuffix))
}
