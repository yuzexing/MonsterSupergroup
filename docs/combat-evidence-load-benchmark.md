...
默认同步到物理快照 `Logs/CombatEvidenceNextValidation/project`，可用 `-ProjectPath` 指定其他独立工程。旧 `Logs/SteamNetworkValidation/project` 的 junction 不能用作隔离证据，新入口会拒绝链接和共享硬链接。必须先结束其他针对同一工程的 Unity 测试，不能同时运行两个进程。脚本不启动真实游戏、不连接 Steam，也不修改游戏画质。快照、脚本版本、命令及前后哈希归档规则见 [续作验证合同](combat-evidence-next-validation.md)。

阵列包含 50、200、500 个 `StatusController`，分别关闭日志、仅本地日志、本地 primary 加一个模拟 remote 持续复制。三种模式的 `endpointCount` 分别为 0、1、2；三端 Host 中继属于独立正确性专项。每组默认 180 秒、重复 3 次，共 81 分钟加初始化、落盘和补传时间；`-Seconds`、`-Repeats`、`-CatchupSeconds` 可调整，补传上限 45 秒，`-Smoke` 设为 1 秒、1 次和最多 15 秒补传。每步按 144 Hz 真实时钟节拍运行，输入时间步按固定规则交替，每个控制器逐次执行，不累计后跳过调用。工作超时不会减少输入数量，而是延长本次实际时间并记录未按时完成的帧数；实际频率不足会使严格 gate 失败。
...
脚本另外生成 `comparison.json`，对照同控制器数、同重复次数的关闭日志结果，严格检查 P95 增量 ≤ 1 ms、P99 增量 ≤ 3 ms、关键和正常负载总丢失为零、预算、实际帧数/频率、无缺组合以及 45 秒内追平。每端必须覆盖有限请求 `[1, producedRecords]`：`produced >= flushed >= producedRecords`、无缺口/写盘失败/待恢复且队列为空；来源尚未关闭时允许未知未来尾部，并以 `coverageInterval` 明示，不能声称完整未来记录已复制。分配不可测单列为 unknown，不以 0 通过完整内存验收。日志目录和每次试验报告均保留，失败也不自动删除。
