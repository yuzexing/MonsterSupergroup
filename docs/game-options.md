...
- **画面：** 窗口／无边框／独占全屏、分辨率、屏幕刷新率（Hz）、垂直同步、帧率上限（FPS）、50%～150% 渲染比例、设备支持的 MSAA、纹理 mipmap 限制。MSAA 主要处理几何边缘，纹理质量只影响支持 mipmap 限制的纹理。

分辨率、屏幕刷新率与帧率上限采用“左箭头—当前值—右箭头”：鼠标点箭头，或选中后按左右键循环切换；Tab／上下键切换控件，选中“应用”后按 Enter 提交。分辨率只显示宽高且不重复，刷新率只列出当前分辨率的可用值。更换分辨率时保留支持的原刷新率，否则选取最接近的可用值；刷新率与 FPS 上限独立。

该交互参考 Nordic Ashes 导出工程中的 `SettingsPanelUI`、`UnityEngine.UI.ArrowOptions` 及 `Menu.unity`：分辨率与 FPS 控件均绑定左右按钮、启用循环并使用 `onChanged`。参考数据没有独立刷新率控件，本项目依照三项拆分要求扩展；参考脚本方法体为空，不把推测的运行行为当成还原结果。应用、保存及 15 秒确认继续使用本项目逻辑。

音量、语言、震动即时应用并保存。画面编辑先保留草稿，点击“应用”才生效；关闭面板丢弃未应用的画面草稿。“恢复此页默认值”只恢复当前页，画面页仍需应用。

显示模式／分辨率／刷新率变更需在 15 秒内确认，否则自动恢复。确认前保存其他设置不会持久化试用中的显示模式。编辑器内禁用显示模式、分辨率和刷新率控件，应在独立运行的游戏中验证。垂直同步开启时禁用 FPS 上限的左右操作，帧率由显示器控制；关闭后恢复先前选择的帧率上限。
...
- `Logs/LocalizationValidation/20260913-182612/results.xml`：24 项 PlayMode 测试通过，覆盖三项独立草稿、分辨率去重、刷新率保留与回退、精确刷新率分数、语言刷新、左右循环、鼠标操作后的键盘焦点以及垂直同步禁用。未应用的修改不改变当前设置。
- 同目录 `tables.log`、`development.log`、`release.log` 记录表格校验和两种构建成功；更新后的程序分别为 `Builds/MenuDevelopment/MonsterSupergroup.exe`、`Builds/MenuRelease/MonsterSupergroup.exe`。
- 最终 Development 包真实窗口复测：鼠标将分辨率由 3840×2160 调至 2560×1600，再按右键恢复；按下键移到刷新率并按左键将 144 Hz 改为 120 Hz，FPS 仍为 144；鼠标将 FPS 调为 240，刷新率仍为 120 Hz。开启垂直同步后 FPS 变灰，继续点击箭头不能修改。Esc 返回首页并丢弃上述草稿，未点击应用。
- 本次真实窗口采用已有保存设置的 3840×2160 独占全屏；截图为 1920×1080 逻辑像素，不能据此算作实际 1280×720 或 1920×1080 窗口验收。三项名称、数值和箭头在该窗口无裁切或重叠。
- 点击首页退出后，进程退出码为 `-1073740771`（`0xC000041D`），退出检查记为失败。日志为 `Logs/DisplaySettings/manual-development.log`，已运行到 `PlayerConnection::Cleanup`；日志没有给出足以确定根因的信息。本次未修改退出流程，不将界面测试与构建成功等同于退出成功。
