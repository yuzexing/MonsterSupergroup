...
测试夹具先由两端分别通过 F5 共用的 Owner 请求升到 2 级，确认不会提升另一玩家，也不会重复发奖，再通过服务器 XP 入口一次补充到 18 级；只固定抽样随机源以保证六种武器覆盖，不改正式升级规则或武器数值。
...
编辑器 Play Mode 或 Development Build 从 Boot 进入 Gameplay 后，聚焦本人 Game 视图／游戏窗口并按 **F5**，每次升一级。此入口只向服务器提交本人的调试请求，服务器按现有 XP 阈值授予一级经验，保留当前经验余量；不会直接修改 Build 或发放卡片。UI 尚未就绪时先排队，准备完成后显示；菜单打开时继续按 F5 会追加逐级奖励，不替换当前候选或 Equipment 目标阶段。按住不连升；失权、断线、组件禁用或死亡时不会发奖，非开发版服务器也不接受该请求。

FMOD 遮挡画面的浮层已在 `FMOD → Edit Settings → Play In Editor → Debug Overlay` 设为 `Disabled`；音频播放、日志级别和 Bank 配置不由这个显示开关改变。
...
- `Logs/M4/f5-final-playmode.xml`：8／8 通过，覆盖一次请求一级、保留 XP 余量、目标阶段继续排队、菜单未就绪时保留奖励以及禁用后不再请求。
- `Logs/M4/Host-20260910-152834-749-p7983/` 和 `Logs/M4/Dedicated-20260910-152834-749-p7984/`：两种组合全部 PASS，五个进程退出码均为 0。两端先后调用 F5 共用的 Owner 请求，服务器验证只提升本人一级，再完成后续 M4 选择与恢复、六种武器表现和退出清理。
- 最终构建日志 `Logs/M4/build-f5-final.log`，产物仍为 `Builds/M4Selection/M4Selection.exe`。可见验收继续使用 `-force-d3d11` 临时参数。
- 进程验证调用 F5 共用入口，不注入物理按键；编辑器的按键人工验收中途停止，未计为通过。FMOD 仅修改 Play In Editor 的 Debug Overlay 开关，现有音频迁移变更另行保留。
